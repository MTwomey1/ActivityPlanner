...
include('Functions.php'); // include functions
        ...
        // make query
        $query = "SELECT plan_id, username, activity, date, location FROM plans WHERE username = ? AND completed = '0' UNION SELECT plans.plan_id, plans.username, plans.activity, plans.date, plans.location FROM plan_users JOIN plans on plans.plan_id = plan_users.plan_id WHERE plan_users.username = ? AND plan_users.status = '1' AND plans.completed = '0'";
